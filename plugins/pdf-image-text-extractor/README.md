...
> Extract text from PDFs and images with format preservation and table detection
...
OCR text extractor for PDFs and images with zero extra dependencies. Scanned PDFs are auto-rendered and processed by AI vision (no tesseract/rapidocr needed); tables extracted via pymupdf's built-in find_tables (no pdfplumber needed); supports batch directory processing.
...
/plugin install pdf-image-text-extractor@redfox-agent-plugins
```
...
codex plugin add pdf-image-text-extractor@redfox-agent-plugins
```
...
2. Search for `pdf-image-text-extractor` (or add the marketplace repo URL first)
3. Click **Install**
</details>
...
```bash
gemini extensions install https://github.com/redfox-data/redfox-agent-plugins --path plugins/pdf-image-text-extractor
# or with Antigravity:
agy plugin install https://github.com/redfox-data/redfox-agent-plugins/plugins/pdf-image-text-extractor
```
...
```bash
cp -r plugins/pdf-image-text-extractor/skills/pdf-image-text-extractor ~/.agents/skills/
```
...
> Extract all text and tables from this PDF: /path/to/document.pdf
...
├── assets/logo.svg
└── skills/pdf-image-text-extractor/
...
- Docs: <https://redfox.hk/skills/pdf-image-text-extractor>
