...
> Watermark-free X/Twitter video downloader with direct MP4 links
...
Paste any X (Twitter) video URL and get a watermark-free direct download link in MP4 format. Works with regular tweets, threads, and quoted videos. Ideal for archiving viral content, saving memes, or backing up your own posts.
...
/plugin install twitter-video-downloader@redfox-agent-plugins
```
...
codex plugin add twitter-video-downloader@redfox-agent-plugins
```
...
2. Search for `twitter-video-downloader` (or add the marketplace repo URL first)
3. Click **Install**
</details>
...
```bash
gemini extensions install https://github.com/redfox-data/redfox-agent-plugins --path plugins/twitter-video-downloader
# or with Antigravity:
agy plugin install https://github.com/redfox-data/redfox-agent-plugins/plugins/twitter-video-downloader
```
...
```bash
cp -r plugins/twitter-video-downloader/skills/twitter-video-downloader ~/.agents/skills/
```
...
> Download the video from this tweet: https://x.com/user/status/xxxxx
...
├── assets/logo.svg
└── skills/twitter-video-downloader/
...
- Docs: <https://redfox.hk/skills/twitter-video-downloader>
